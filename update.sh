...
XMLSIZE=`du -c dataflat/xml|tail -1|tr -dc '0-9'`

echo "xml size $XMLSIZE"

# check we have at least 1gig of data otherwise something is wrong
if (( 1000000 > XMLSIZE )); then
	echo "xml size is too small, keeping last known good data"
	exit 0
fi
